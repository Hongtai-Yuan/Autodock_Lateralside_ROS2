...
void autodock_controller::searching_state_fun(){
    if (!tag_y) tag_y = 0.5;
    neuron_forward(blind_sign*fabs(tag_y/1.5));
...
void autodock_controller::approach_state_fun(){
    ...
    if (in_view && M_PI-fabs(tag_yaw)>pose_set.theta_bounds){
    ...
    else if((!in_view) && (M_PI-fabs(tag_yaw)>pose_set.theta_bounds)){
        // neuron_stop();
        if (!tag_y) tag_y = 0.5;
        blind_sign = -sign(tag_y);
        neuron_forward(blind_sign*fabs(tag_y/3.0));
        set_docking_state("centering");
    }
    else{
        neuron_stop();
